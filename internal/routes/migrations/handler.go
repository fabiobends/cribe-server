...
func (handler *MigrationHandler) HandleRequest(w http.ResponseWriter, r *http.Request) {
	...
	handler.logger.Debug("Migration request received", map[string]interface{}{
		"method":     r.Method,
		"path":       r.URL.Path,
	...
	if len(paths) > 1 {
		handler.logger.Warn("Invalid migration route - extra path segments", map[string]interface{}{
			"paths": paths,
		})
		utils.NotFound(w, r)
		return
	...
	default:
		handler.logger.Warn("Method not allowed for migrations endpoint", map[string]interface{}{
			"method": r.Method,
		})
		utils.NotAllowed(w)
	}
...
func (handler *MigrationHandler) handleGet(w http.ResponseWriter, _ *http.Request) {
	...
	handler.logger.Info("Migration dry run completed", map[string]interface{}{
...
func (handler *MigrationHandler) handlePost(w http.ResponseWriter, _ *http.Request) {
	...
	if len(migrations) == 0 {
		handler.logger.Info("Migration live run completed - no migrations applied", map[string]interface{}{
			"migrationsCount": 0,
		})
		utils.EncodeResponse(w, http.StatusOK, migrations)
	} else {
		handler.logger.Info("Migration live run completed - migrations applied", map[string]interface{}{
			"migrationsCount": len(migrations),
		})
